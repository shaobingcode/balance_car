...
uint8_t display_buf[20] = "Init Success";
uint32_t sys_tick;
...
int main(void)
{
    ...
    MPU_Init();
    mpu_dmp_init();
    OLED_ShowString(1,1,display_buf,11);
    ...
    Motor_PWM_Ctrl(10,10);
    OLED_Clear();
    /* USER CODE END 2 */

    while (1)
    {
        sprintf((char *)display_buf,"Encoder_L:%d   ",Left_Encoder_Speed);
        OLED_ShowString(0,0,display_buf,16);
        sprintf((char *)display_buf,"Encoder_R:%d   ",Right_Encoder_Speed);
